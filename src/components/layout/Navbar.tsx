...
import { Download, Menu, X, House } from "lucide-react";
...
const Navbar = () => {
  ...
  const isIndexPage =
    pathname === base || pathname === `${base}/index.html` || pathname === "";

  // Index page: navbar shrinks from 95% → 50% on scroll (original behaviour)
  const indexNavWidth = useTransform(
    scrollYProgress,
    [0, 0.1, 1],
    ["95%", "50%", "50%"],
  );
  ...
  // ── Sub-page navbar — fixed 30% width, centered, logo + Home only ────────
  if (!isIndexPage) {
    return (
      <>
        <motion.div
        ...
        <div className='fixed top-0 left-0 right-0 z-40 flex justify-center pt-4 px-4'>
          <motion.nav
            ...
            style={{ width: "30%", minWidth: 180, padding: "0.65rem 1.25rem" }}
          ...
          >
            <div className='flex items-center justify-between gap-4'>
              ...
              <a
                ...
                className='font-display text-lg font-bold text-foreground leading-none'
              >
                DM
              </a>
              ...
              <motion.a
                href={`${import.meta.env.BASE_URL}`}
                className='flex items-center gap-1.5 px-3 py-1.5 rounded-full glass text-sm font-medium text-muted-foreground hover:text-foreground transition-colors duration-200'
      ...
      <div className='fixed top-0 left-0 right-0 z-40 flex justify-center pt-4 px-4'>
        ...
        >
          <div className='flex items-center justify-between px-4 md:px-6'>
            ...
            {/* Desktop nav items — index page only */}
            {isIndexPage && (
              <div className='hidden md:flex items-center gap-6 lg:gap-8'>
                {navItems.map((item) => (
                  <a
